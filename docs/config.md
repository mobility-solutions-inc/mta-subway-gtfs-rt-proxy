...
| `$SCHEDULE_FEED_REFRESH_INTERVAL`     | `15 * 60`                                  | s    | How often to check if MTA's Schedule feed has changed, minus the duration it took last time to check. Conditional HTTP requests are used when MTA supplies validators.                                                 |
| `$SCHEDULE_FEED_REFRESH_MIN_INTERVAL` | `60`                                       | s    | Minimum time to wait between attempts to check if MTA's Schedule feed has changed. Effectively a lower bound for `$SCHEDULE_FEED_REFRESH_INTERVAL`.                                                                    |
| `$SCHEDULE_FEED_STALE_AFTER`          | `2 * 60 * 60`                              | s    | Maximum time since a successful upstream check before `/health` reports unhealthy.                                                                                                                                     |
| `$SCHEDULE_FEED_USED_RETENTION`       | `24 * 60 * 60`                             | s    | How long to retain a schedule digest after its last schedule or realtime request. The latest digest is always retained.                                                                                                |
...
| `$REALTIME_FEED_FETCH_INTERVAL`     | `30`                                   | s    | How often to fetch MTA's Realtime feeds, minus the duration it took last time.                                                                                                                                                            |
| `$REALTIME_FEED_FETCH_MIN_INTERVAL` | `30`                                   | s    | Minimum time to wait between fetches of each MTA Realtime feed. Effectively a lower bound for `$REALTIME_FEED_FETCH_INTERVAL`.                                                                                                            |
